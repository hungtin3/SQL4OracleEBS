--Setting Concurrent Timeout: Profile value
--2021-Jan-12
DECLARE
   v_Check        BOOLEAN;
      ...
      SELECT p.Profile_Option_Name Short_Name,
             ...
             (CASE
                WHEN p.Profile_Option_Name =
                     'CONC_PP_PROCESS_TIMEOUT' AND
                     v.Profile_Option_Value =
                     '10800' THEN
                 'GOOD'
                WHEN p.Profile_Option_Name =
                     'CONC_PP_RESPONSE_TIMEOUT' AND
                     v.Profile_Option_Value =
                     '10800' THEN
                 'GOOD'
             END) Good_Or_Bad
       ...
       WHERE p.Profile_Option_Id =
            ...
            --AND v.Level_Id = 10004   --'Site',10002,'Application',10003,'Responsibility',10004,'User',10005,'Server','UnDef'
             AND
             (Upper(n.User_Profile_Option_Name) LIKE
             Upper('Concurrent:%OPP%Timeout%%')) AND
...
BEGIN
   ...
   FOR c IN c_Profile LOOP
      IF c.Short_Name =
         'CONC_PP_PROCESS_TIMEOUT' THEN
         v_Value := '10800';
      ELSIF c.Short_Name =
            'CONC_PP_RESPONSE_TIMEOUT' THEN
         v_Value := '10800';
      ELSE
         v_Value := NULL;
      END IF;
